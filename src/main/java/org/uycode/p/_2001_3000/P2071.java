...
public class P2071 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(maxTaskAssign(tasks, workers, 139, 2075));
    }

    public static int maxTaskAssign(int[] tasks, int[] workers, int pills, int strength) {
/*
        List<Integer> t = Arrays.stream(tasks).boxed().toList();
        List<Integer> w = Arrays.stream(workers).boxed().toList();*/

        Arrays.sort(tasks);
        Arrays.sort(workers);
        int minIndex = tasks.length - 1;
        int[] hasPilled = new int[workers.length];
        int ans = 0;
        for (int i = workers.length - 1; i >= 0; i--) {
            for (int j = minIndex; j >= 0; j--) {
                if (workers[i] >= tasks[j]) {
                    ans++;
                    tasks[j] = 1_000_000_001;
                    minIndex = j - 1;
                    hasPilled[i] = 1;
                    break;
                }
            }
            if (hasPilled[i] == 0 && pills > 0) {
                workers[i] += strength;
                hasPilled[i] = 1;
                i++;
                pills--;
                minIndex = tasks.length - 1;
            }
        }
